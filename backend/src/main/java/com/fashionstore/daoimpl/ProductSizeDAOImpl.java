...
public class ProductSizeDAOImpl implements ProductSizeDAO {
    ...
    public boolean reduceStock(Connection conn, int productId, String sizeLabel, int quantity) {
        ...
        // Use row-level locking with SELECT FOR UPDATE
        String selectSql = "SELECT product_size_id, stock_quantity FROM product_sizes " +
                          "WHERE product_id = ? AND size_label = ? AND stock_quantity >= ? " +
                          "FOR UPDATE";
        String updateSql = "UPDATE product_sizes " +
                          "SET stock_quantity = stock_quantity - ?, " +
                          "    is_available = CASE WHEN (stock_quantity - ?) > 0 THEN true ELSE false END " +
    ...
    @Override
    public boolean increaseStock(int productId, String sizeLabel, int quantity) {
        if (quantity <= 0) {
            return false;
        }
